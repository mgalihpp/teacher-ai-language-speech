...
const TypingBox = () => {
  const { loading, messages, setMessages, setLoading, playAudioTTS } = useAiTeacher();
  ...
  return (
    <div
      className="z-10 flex max-w-[600px] flex-col space-y-6 rounded-xl 
    ...
    >
      ...
      {loading ? (
        ...
        <div className="flex gap-3">
          ...
          p-2 px-4 text-white shadow-inner shadow-slate-900/60 placeholder:text-white/50 focus:outline focus:outline-white/80
          "
          ...
          <button
            onClick={ask}
            className="rounded-full bg-slate-100/20 p-2 px-6 text-white"
          >
            Ask
          </button>
        </div>
